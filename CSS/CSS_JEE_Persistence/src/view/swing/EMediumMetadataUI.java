			...
			public void mouseClicked(MouseEvent event) {
				...
				if (selectionPath != null) {
					...
					if (selection.getParent() == annotations) { 
						...
						annotationTextArea.setText(EMediumMetadataUI.this
								.delegate.getAnnotationText(pageNum, selectedAnnotation));
					} 
					...
					if (event.getClickCount() >= 2)
						if (selection.getParent() == bookmarks && documentUI != null)
							try {
								...
								JOptionPane.showMessageDialog(frame, "Invalid bookmark number", 
										"Invalid bookmark", JOptionPane.ERROR_MESSAGE);
							} catch (NoSuchPageException e) {
								JOptionPane.showMessageDialog(frame, "No shuch page", 
										"Error reading page", JOptionPane.ERROR_MESSAGE);
							}
				}
			}
		};
